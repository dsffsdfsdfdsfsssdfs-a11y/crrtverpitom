...
type Content=any;
const setPath=(o:any,path:string,value:any)=>{const p=path.split('.');let x=o;for(let i=0;i<p.length-1;i++)x=x[p[i]];x[p.at(-1)!]=value};
const getPath=(o:any,path:string)=>path.split('.').reduce((x:any,k)=>x?.[k],o);
...
async function optimizeImage(file:File){
 ...
 try{
  ...
  const maxSide=2200,scale=Math.min(1,maxSide/Math.max(bitmap.width,bitmap.height));
  ...
  const blob=await new Promise<Blob|null>(ok=>canvas.toBlob(ok,'image/webp',.84));
  if(!blob||blob.size>=file.size)return file;
  return new File([blob],file.name.replace(/\.[^.]+$/,'')+'.webp',{type:'image/webp'});
 }catch{return file}
}
export default function Admin(){
 const [password,setPassword]=useState(''),[content,setContent]=useState<Content|null>(null),[message,setMessage]=useState(''),[uploading,setUploading]=useState(0);const previewRef=useRef<HTMLIFrameElement>(null);
 ...
 async function save(){if(uploading){setMessage('Дождись окончания загрузки фотографии.');return}setMessage('Сохраняю…');const r=await fetch('/api/admin/save',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(content)});setMessage(r.ok?'Сохранено. Изменения уже опубликованы.':(await r.json()).error)}
 async function upload(index:number,file:File,target?:string){
  ...
  change(path,preview);
  setMessage('Фото уже показано в предпросмотре. Загружаю на сервер…');
  setUploading(x=>x+1);
  try{
    const optimized=await optimizeImage(file);
    const encoded=await readDataUrl(optimized);
    const r=await fetch('/api/admin/upload',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:optimized.name,base64:encoded.split(',')[1]})});
    const data=await r.json();
    if(!r.ok||!data.url)throw new Error(data.error||'Ошибка загрузки');
    change(path,data.url+'?v='+Date.now());
    setMessage('Фото загружено. Предпросмотр обновлён — теперь можно сохранить.');
  }catch(error){
    change(path,previous);
    setMessage(error instanceof Error?error.message:'Ошибка загрузки');
  }finally{setUploading(x=>Math.max(0,x-1))}
}
 ...
 return <main className="admin-shell"><header className="admin-head"><a href="/">← Открыть сайт</a><div><b>Редактор сайта</b><button onClick={save} className="gold-btn" disabled={uploading>0}>{uploading?`Загрузка фото… (${uploading})`:'Сохранить изменения'}</button></div></header><div className="editor-layout"><div className="admin-wrap"><p className="admin-message">{message||'Меняй всё нужное и нажми «Сохранить изменения». Изменения публикуются сразу.'}</p>
