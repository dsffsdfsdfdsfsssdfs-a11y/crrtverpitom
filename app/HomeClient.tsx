...
export default function HomeClient({initialContent}:{initialContent:any}) {
  ...
  const editorTextStyle=(key:string,defaultSize=100,defaultColor?:string)=>({
    transform:`translate(${clamp(num(c.editor?.[key+'X'],0),-1500,1500)}px,${clamp(num(c.editor?.[key+'Y'],0),-1500,1500)}px)`,
    fontSize:`${clamp(num(c.editor?.[key+'Size'],defaultSize),10,300)}%`,
    fontFamily:c.editor?.[key+'Font']?`${c.editor[key+'Font']}, Georgia, serif`:undefined,
    color:c.editor?.[key+'Color']||defaultColor,
    letterSpacing:`${num(c.editor?.[key+'Spacing'],0)}em`,
    position:'relative',
    zIndex:2
  } as React.CSSProperties);
  ...
  return <>
    ...
    <header className="topbar" data-editor-key="headerBg" style={{backgroundColor:c.header.bgColor||undefined,color:c.header.textColor||undefined}}>
      <a className="brand header-brand" href="/" aria-label="Обновить страницу" onClick={(e)=>{e.preventDefault();window.location.reload()}}><span className="brand-logo-slot" style={{width:logoSlot}}>{logoSrc&&<span data-editor-key="logo"><Image src={logoSrc} alt="" aria-hidden="true" priority width={160} height={160} quality={65} className="brand-image brand-image-img" style={{width:logoSize,height:logoSize,transform:`translate(${logoX}px,${logoY}px)`}}/></span>}</span><span className="header-brush-signature-wrap" aria-hidden="true"><span data-editor-key="brushText1" className="header-brush-signature header-brush-signature-1" style={{fontSize:`${2.2*clamp(num(c.header.brush1Size,55),20,180)/100}rem`,transform:`translate(${clamp(num(c.header.brush1X,0),-1500,1500)}px,${clamp(num(c.header.brush1Y,0),-1500,1500)}px) rotate(${clamp(num(c.header.brush1Rotate,-2),-180,180)}deg)`,color:c.header.brush1Color||undefined,letterSpacing:`${num(c.header.brush1Spacing,0)}em`}}>{brush1Text}</span><span data-editor-key="brushText2" className="header-brush-signature header-brush-signature-2" style={{fontSize:`${2.2*clamp(num(c.header.brush2Size,55),20,180)/100}rem`,transform:`translate(${clamp(num(c.header.brush2X,72),-1500,1500)}px,${clamp(num(c.header.brush2Y,34),-1500,1500)}px) rotate(${clamp(num(c.header.brush2Rotate,0),-180,180)}deg)`,color:c.header.brush2Color||undefined,letterSpacing:`${num(c.header.brush2Spacing,0)}em`}}>{brush2Text}</span></span><span data-editor-key="brandText" className="brand-copy" style={{fontSize:`${c.header.textSize}%`,fontFamily:`${c.header.textFont||'Manrope'}, Arial, sans-serif`,transform:`translate(${textX}px,${textY}px)`,color:c.header.textColor||undefined,letterSpacing:`${num(c.header.textSpacing,0)}em`}}>{c.header.title}<br/><b>{c.header.subtitle}</b></span></a>
        ...
        <div className="specialty-modern-grid">
          ...
          <div className="specialty-modern-right">
            <div className="specialty-modern-note">
              ...
              <div className="specialty-modern-tags">{(c.specialty.formats||['144 ячеек','104 ячеек','96 ячеек','Р9']).map((x:string)=><span key={x}>{x}</span>)}</div>
            </div>
          </div>
        </div>
      </div>
    </section>
    <section className="photo-band greenhouse" data-editor-key="greenhouse" style={{backgroundImage:`linear-gradient(0deg,#180e0bcf,transparent 75%),url(${optimizeImage(c.greenhouse.image)})`}}><div><p data-editor-key="greenhouseKicker" className="eyebrow" style={editorTextStyle('greenhouseKicker',100)}>ПРОИЗВОДСТВО</p><h2><span data-editor-key="greenhouseTitle" style={{...editorTextStyle('greenhouseTitle',100),display:'inline-block'}}>{c.greenhouse.title}</span><br/><em><span data-editor-key="greenhouseAccent" style={{...editorTextStyle('greenhouseAccent',100),display:'inline-block'}}>{c.greenhouse.accent}</span></em></h2><p data-editor-key="greenhouseText" style={editorTextStyle('greenhouseText',100)}>{c.greenhouse.text}</p></div></section>
    <section className="assortment wrap" id="assortment" data-editor-key="assortment"><div className="section-head"><div><p data-editor-key="assortmentKicker" className="eyebrow gold" style={editorTextStyle('assortmentKicker',100)}>{c.assortmentKicker||'КОЛЛЕКЦИЯ'}</p><h2 data-editor-key="assortmentTitle" style={editorTextStyle('assortmentTitle',100)}>{c.assortmentHeading||<>Ассортимент<br/>для красивых садов.</>}</h2></div><a className="outline-btn" href={c.priceUrl}>Скачать прайс <span>↓</span></a></div><div className="plant-grid">{c.assortment.map((x:string,i:number)=><article key={x} className={'plant p'+i}><span>0{i+1}</span><h3>{x}</h3><p>{i===5?'Вейгела · ива · лапчатка · рябинник · снежноягодник':'Сортовые растения собственного производства'}</p><i>↗</i></article>)}</div></section>
    <section className="mother" data-editor-key="mother" style={{backgroundImage:`linear-gradient(90deg,#281914e8,#28191424),url(${optimizeImage(c.mother.image)})`}}><div className="wrap mother-copy"><p data-editor-key="motherKicker" className="eyebrow" style={editorTextStyle('motherKicker',100)}>ОСНОВА КАЧЕСТВА</p><h2><span data-editor-key="motherTitle" style={{...editorTextStyle('motherTitle',100),display:'inline-block'}}>{c.mother.title}</span><br/><em><span data-editor-key="motherAccent" style={{...editorTextStyle('motherAccent',100),display:'inline-block'}}>{c.mother.accent}</span></em></h2><p data-editor-key="motherText" style={editorTextStyle('motherText',100)}>{c.mother.text}</p></div></section>
    <section className="gallery wrap" id="gallery" data-editor-key="gallery"><div className="section-head"><div><p data-editor-key="galleryKicker" className="eyebrow gold" style={editorTextStyle('galleryKicker',100)}>{c.galleryKicker||'НАШИ РАСТЕНИЯ'}</p><h2 data-editor-key="galleryTitle" style={editorTextStyle('galleryTitle',100)}>{c.galleryHeading||<>Фотографии<br/>наших черенков.</>}</h2></div><a className="line-link" href="#order">Открыть галерею <span>→</span></a></div><div className="gallery-grid">{c.galleryImages.map((url:string,i:number)=>{const s=c.galleryImageSettings?.[i]||{scale:100,x:50,y:50};return <div key={url} style={{backgroundImage:`url(${optimizeImage(url)})`,backgroundSize:`${s.scale}%`,backgroundPosition:`${s.x}% ${s.y}%`}}/>})}</div></section>
    ...
    <section className="knowledge wrap" id="knowledge" data-editor-key="knowledge"><p data-editor-key="knowledgeKicker" className="eyebrow gold" style={editorTextStyle('knowledgeKicker',100)}>{c.knowledgeKicker||'ДЕЛИМСЯ ОПЫТОМ'}</p><h2 data-editor-key="knowledgeTitle" style={editorTextStyle('knowledgeTitle',100)}>{c.knowledgeHeading||'Полезная информация'}</h2><div className="knowledge-list">{c.resources.map((x:{title:string,url:string},i:number)=><a href={x.url} key={x.title}><span>0{i+1}</span><b>{x.title}</b><i>↗</i></a>)}</div></section>
    <section className="order wrap" id="order" data-editor-key="order"><div><p data-editor-key="orderKicker" className="eyebrow" style={editorTextStyle('orderKicker',100)}>{c.order?.kicker||'СДЕЛАЕМ ПОДБОРКУ'}</p><h2><span data-editor-key="orderTitle" style={{...editorTextStyle('orderTitle',100),display:'inline-block'}}>{c.order?.title||'Готовы выбрать'}</span><br/><em><span data-editor-key="orderAccent" style={{...editorTextStyle('orderAccent',100),display:'inline-block'}}>{c.order?.accent||'растения?'}</span></em></h2></div><div><p data-editor-key="orderText" style={editorTextStyle('orderText',100)}>{c.order?.text||'Напишите нам — подберём культуры, объём и формат поставки для вашей задачи.'}</p><button className="gold-btn" onClick={()=>setOrder(true)}>{c.order?.button||'Оформить заказ'} <span>↗</span></button><div className="help"><a href={'tel:+'+c.phoneLink}>{c.phone}</a><span>{c.order?.help||'Ответим на вопросы и поможем, если возникла проблема.'}</span></div></div></section>
