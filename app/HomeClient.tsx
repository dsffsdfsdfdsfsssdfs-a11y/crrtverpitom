...
export default function HomeClient({initialContent}:{initialContent:any}) {
  ...
  return <>
    ...
    <section className="photo-band greenhouse" data-editor-key="greenhouse" style={{backgroundImage:`linear-gradient(0deg,#180e0bcf,transparent 75%),url(${optimizeImage(c.greenhouse.image)})`}}><div><p data-editor-key="greenhouseKicker" className="eyebrow" style={editorTextStyle('greenhouseKicker',100)}>ПРОИЗВОДСТВО</p><h2 data-editor-key="greenhouseTitle" style={editorTextStyle('greenhouseTitle',100)}>{c.greenhouse.title}</h2><h2 data-editor-key="greenhouseAccent" style={editorTextStyle('greenhouseAccent',100)}><em>{c.greenhouse.accent}</em></h2><p data-editor-key="greenhouseText" style={editorTextStyle('greenhouseText',100)}>{c.greenhouse.text}</p></div></section>
    ...
    <section className="mother" data-editor-key="mother" style={{backgroundImage:`linear-gradient(90deg,#281914e8,#28191424),url(${optimizeImage(c.mother.image)})`}}><div className="wrap mother-copy"><p data-editor-key="motherKicker" className="eyebrow" style={editorTextStyle('motherKicker',100)}>ОСНОВА КАЧЕСТВА</p><h2 data-editor-key="motherTitle" style={editorTextStyle('motherTitle',100)}>{c.mother.title}</h2><h2 data-editor-key="motherAccent" style={editorTextStyle('motherAccent',100)}><em>{c.mother.accent}</em></h2><p data-editor-key="motherText" style={editorTextStyle('motherText',100)}>{c.mother.text}</p></div></section>
    ...
    <section className="order wrap" id="order" data-editor-key="order"><div><p data-editor-key="orderKicker" className="eyebrow" style={editorTextStyle('orderKicker',100)}>{c.order?.kicker||'СДЕЛАЕМ ПОДБОРКУ'}</p><h2 data-editor-key="orderTitle" style={editorTextStyle('orderTitle',100)}>{c.order?.title||'Готовы выбрать'}</h2><h2 data-editor-key="orderAccent" style={editorTextStyle('orderAccent',100)}><em>{c.order?.accent||'растения?'}</em></h2></div><div><p data-editor-key="orderText" style={editorTextStyle('orderText',100)}>{c.order?.text||'Напишите нам — подберём культуры, объём и формат поставки для вашей задачи.'}</p><button className="gold-btn" onClick={()=>setOrder(true)}>{c.order?.button||'Оформить заказ'} <span>↗</span></button><div className="help"><a href={'tel:+'+c.phoneLink}>{c.phone}</a><span>{c.order?.help||'Ответим на вопросы и поможем, если возникла проблема.'}</span></div></div></section>
