...
export default function HomeClient({initialContent}:{initialContent:any}) {
  ...
  const brushLines=String(c.header.brushText||'Центр размножения\nрастений').split('\n');
  ...
  return <>
    ...
    <header className="topbar" data-editor-key="headerBg" style={{backgroundColor:c.header.bgColor||undefined,color:c.header.textColor||undefined}}>
      <a className="brand header-brand" href="/" aria-label="Обновить страницу" onClick={(e)=>{e.preventDefault();window.location.reload()}}><span className="brand-logo-slot" style={{width:logoSlot}}>{logoSrc&&<span data-editor-key="logo"><Image src={logoSrc} alt="" aria-hidden="true" priority width={160} height={160} quality={65} className="brand-image brand-image-img" style={{width:logoSize,height:logoSize,transform:`translate(${logoX}px,${logoY}px)`}}/></span>}</span><span data-editor-key="brushText" className="header-brush-signature" aria-hidden="true" style={{fontSize:`${2.2*clamp(num(c.header.brushSize,55),20,180)/100}rem`,transform:`translate(${clamp(num(c.header.brushX,0),-1500,1500)}px,${clamp(num(c.header.brushY,0),-1500,1500)}px) rotate(-1.8deg)`,color:c.header.brushColor||undefined,letterSpacing:`${num(c.header.brushSpacing,0)}em`}}>{brushLines.map((line:string,i:number)=><span key={i}>{line}</span>)}</span><span data-editor-key="brandText" className="brand-copy" style={{fontSize:`${c.header.textSize}%`,fontFamily:`${c.header.textFont||'Manrope'}, Arial, sans-serif`,transform:`translate(${textX}px,${textY}px)`,color:c.header.textColor||undefined,letterSpacing:`${num(c.header.textSpacing,0)}em`}}>{c.header.title}<br/><b>{c.header.subtitle}</b></span></a>
