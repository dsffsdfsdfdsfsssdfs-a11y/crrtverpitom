...
export default function HomeClient({initialContent}:{initialContent:any}) {
  ...
  return <>
    ...
    <header className="topbar" data-editor-key="headerBg" style={{backgroundColor:c.header.bgColor||undefined,color:c.header.textColor||undefined}}>
      <a className="brand header-brand" href="/" aria-label="Обновить страницу" onClick={(e)=>{e.preventDefault();window.location.reload()}}><span className="brand-logo-slot" style={{width:logoSlot}}>{logoSrc&&<span data-editor-key="logo"><Image src={logoSrc} alt="" aria-hidden="true" priority width={160} height={160} quality={65} className="brand-image brand-image-img" style={{width:logoSize,height:logoSize,transform:`translate(${logoX}px,${logoY}px)`}}/></span>}</span><span className="header-brush-signature" aria-hidden="true"><span>Центр</span><span>размножения</span><span>растений</span></span><span data-editor-key="brandText" className="brand-copy" style={{fontSize:`${c.header.textSize}%`,fontFamily:`${c.header.textFont||'Manrope'}, Arial, sans-serif`,transform:`translate(${textX}px,${textY}px)`,color:c.header.textColor||undefined,letterSpacing:`${num(c.header.textSpacing,0)}em`}}>{c.header.title}<br/><b>{c.header.subtitle}</b></span></a>
