...
export default function HomeClient({initialContent}:{initialContent:any}) {
  ...
  const [c,setContent]=useState(initialContent);
  ...
  const logoSize=clamp(n(c.header.logoSize,53),28,110);
  const logoX=clamp(n(c.header.logoX,0),-50,70);
  const logoY=clamp(n(c.header.logoY,0),-24,24);
  const headerTextX=clamp(n(c.header.textX,0),-40,80);
  const headerTextY=clamp(n(c.header.textY,0),-24,24);
  const heroTextX=clamp(n(c.hero.textX,0),-140,220);
  const heroTextY=clamp(n(c.hero.textY,0),-30,300);
  const logoSlot=Math.max(70,logoSize+Math.abs(logoX)+18);
