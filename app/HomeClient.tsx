...
export default function HomeClient({initialContent}:{initialContent:any}) {
  ...
  return <>
    ...
    <section className="specialty-modern" id="about" data-editor-key="specialty">
      <div className="specialty-modern-inner">
        <div className="specialty-modern-top">
          <p className="specialty-modern-kicker">{c.specialty.kicker||'НАША СПЕЦИАЛИЗАЦИЯ'}</p>
        </div>
        <div className="specialty-modern-grid">
          ...
          <div className="specialty-modern-right">
            <div className="specialty-modern-note">
              <span className="specialty-modern-note-title">{c.specialty.noteTitle||'Выращиваем укоренённые черенки'}</span>
              <p className="specialty-modern-note-text">{c.specialty.paragraph2}</p>
    ...
    <section className="photo-band greenhouse" data-editor-key="greenhouse" style={{backgroundImage:`linear-gradient(0deg,#180e0bcf,transparent 75%),url(${optimizeImage(c.greenhouse.image)})`}}><div><p className="eyebrow">ПРОИЗВОДСТВО</p><h2>{c.greenhouse.title}<br/><em>{c.greenhouse.accent}</em></h2><p>{c.greenhouse.text}</p></div></section>
    <section className="assortment wrap" id="assortment" data-editor-key="assortment"><div className="section-head"><div><p className="eyebrow gold">{c.assortmentKicker||'КОЛЛЕКЦИЯ'}</p><h2>{c.assortmentHeading||<>Ассортимент<br/>для красивых садов.</>}</h2></div><a className="outline-btn" href={c.priceUrl}>Скачать прайс <span>↓</span></a></div><div className="plant-grid">{c.assortment.map((x:string,i:number)=><article key={x} className={'plant p'+i}><span>0{i+1}</span><h3>{x}</h3><p>{i===5?'Вейгела · ива · лапчатка · рябинник · снежноягодник':'Сортовые растения собственного производства'}</p><i>↗</i></article>)}</div></section>
    <section className="mother" data-editor-key="mother" style={{backgroundImage:`linear-gradient(90deg,#281914e8,#28191424),url(${optimizeImage(c.mother.image)})`}}><div className="wrap mother-copy"><p className="eyebrow">ОСНОВА КАЧЕСТВА</p><h2>{c.mother.title}<br/><em>{c.mother.accent}</em></h2><p>{c.mother.text}</p></div></section>
    <section className="gallery wrap" id="gallery" data-editor-key="gallery"><div className="section-head"><div><p className="eyebrow gold">{c.galleryKicker||'НАШИ РАСТЕНИЯ'}</p><h2>{c.galleryHeading||<>Фотографии<br/>наших черенков.</>}</h2></div><a className="line-link" href="#order">Открыть галерею <span>→</span></a></div><div className="gallery-grid">{c.galleryImages.map((url:string,i:number)=>{const s=c.galleryImageSettings?.[i]||{scale:100,x:50,y:50};return <div key={url} style={{backgroundImage:`url(${optimizeImage(url)})`,backgroundSize:`${s.scale}%`,backgroundPosition:`${s.x}% ${s.y}%`}}/>})}</div></section>
    ...
    <section className="knowledge wrap" id="knowledge" data-editor-key="knowledge"><p className="eyebrow gold">{c.knowledgeKicker||'ДЕЛИМСЯ ОПЫТОМ'}</p><h2>{c.knowledgeHeading||'Полезная информация'}</h2><div className="knowledge-list">{c.resources.map((x:{title:string,url:string},i:number)=><a href={x.url} key={x.title}><span>0{i+1}</span><b>{x.title}</b><i>↗</i></a>)}</div></section>
    <section className="order wrap" id="order" data-editor-key="order"><div><p className="eyebrow">{c.order?.kicker||'СДЕЛАЕМ ПОДБОРКУ'}</p><h2>{c.order?.title||'Готовы выбрать'}<br/><em>{c.order?.accent||'растения?'}</em></h2></div><div><p>{c.order?.text||'Напишите нам — подберём культуры, объём и формат поставки для вашей задачи.'}</p><button className="gold-btn" onClick={()=>setOrder(true)}>{c.order?.button||'Оформить заказ'} <span>↗</span></button><div className="help"><a href={'tel:+'+c.phoneLink}>{c.phone}</a><span>{c.order?.help||'Ответим на вопросы и поможем, если возникла проблема.'}</span></div></div></section>
